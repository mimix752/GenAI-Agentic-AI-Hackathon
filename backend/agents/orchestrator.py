...
class OrchestratorAgent:
    ...
    def process_single_cv(self, cv_file: str, job_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # interview_questions = self.interview.run(extracted_data, analysis_result)
            interview_questions = [
                "Parlez-moi de votre expérience en Python",
    ...
    def run(self, cv_files: List[str], job_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"✅ Description du poste validée ({len(job_description)} caractères)\n")
        ...
        successful_analyses = [c for c in candidates if c.get("status") == "success"]
        avg_score = sum(c.get("score", 0) for c in successful_analyses) / len(successful_analyses) if successful_analyses else 0
        
        print("\n" + "="*60)
        print("📊 RÉSULTATS GLOBAUX")
        print("="*60)
        print(f"✅ CVs traités avec succès: {len(successful_analyses)}/{len(candidates)}")
        print(f"📈 Score moyen: {avg_score:.1f}/100")
        if candidates_sorted:
            print(f"🏆 Meilleur candidat: {candidates_sorted[0].get('name')} ({candidates_sorted[0].get('score')}/100)")
        print("="*60 + "\n")
...
if __name__ == "__main__":
    ...
    test_cv_files = [
        "data/cv_candidat1.pdf",
        "data/cv_candidat2.txt"
    ]
